...
function updateTestResultMapForSnapshot(resultMap: Map<string, any>, snapshot: any) {
    for (const taskState of snapshot.taskStates) {
        resultMap.set(snapshot.id, taskState.state);
    }
    snapshot.children.forEach((child: any) => updateTestResultMapForSnapshot(resultMap, child));
}

export function updateTestResultMap(
    resultMap: Map<string, any>,
    testResults: readonly any[],
): void {
    const ordered = new Array(...testResults).sort((a, b) => a.completedAt - b.completedAt);
    ordered.forEach((testResult) => {
        testResult.results.forEach((snapshot: any) => updateTestResultMapForSnapshot(resultMap, snapshot));
    });
}

export function checkForFailedTests(resultMap: Map<string, any>): boolean {
    return (
        Array.from(resultMap.values()).find(
            (state) => state === 'failed' || state === 'errored',
        ) !== undefined
    );
}
